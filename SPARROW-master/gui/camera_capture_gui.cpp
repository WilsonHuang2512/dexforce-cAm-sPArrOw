...
bool CameraCaptureGui::saveOneFrameData(QString path_name)
{
	...
	if (SaveDataType::Undistort == save_data_type_)
	{
		...
		if (SparrowPixelType::BayerRG8 == pixel_type_)
		{
...
void CameraCaptureGui::setUiData()
{
	...
	{
	case (int)SparrowEngine::Normal:
	{
		ui.comboBox_engine->setCurrentIndex((int)SparrowEngine::Normal);
	}
	break;
	case (int)SparrowEngine::Reflect:
	{
		ui.comboBox_engine->setCurrentIndex((int)SparrowEngine::Reflect); 
	}
	break;
	case (int)SparrowEngine::Black:
	{
		ui.comboBox_engine->setCurrentIndex((int)SparrowEngine::Black);
	}
	break;
	default:
		break;
...
void CameraCaptureGui::captureOneFrameBaseThread(bool hdr)
{
	...
	if (0 == ret_code)
	{
		...
		{
			DfGetColorBrightnessData(color_brightness.data, SparrowColor::Bgr);
			color_brightness_map_ = color_brightness.clone();

			undistort_color_brightness_map_ = color_brightness.clone();
		
			DfGetUndistortColorBrightnessData(undistort_color_brightness_map_.data, SparrowColor::Bgr);
...
void  CameraCaptureGui::do_pushButton_connect()
{
	...
	do {
		...
		/*	if (!rx2.exactMatch(camera_ip_))
			...
			if (camera_ip_ != "127.0.0.1")
			{

				addLogMessage(tr("IP无效，请输入固定IP:127.0.0.1"));
				break;
			}
			...
			if (DF_SUCCESS == ret_code)
			{
				...
				if (DF_SUCCESS == ret_code)
				{
					pixel_type_ = SparrowPixelType(type);
				}
				else
				{
...
void CameraCaptureGui::do_undate_show_slot()
{
	...
	if (SparrowPixelType::Mono == pixel_type_)
	{
		renderBrightnessImage(brightness_map_);
	}
	else if(SparrowPixelType::BayerRG8 == pixel_type_)
	{
...
void CameraCaptureGui::do_comboBox_activated_engine(int index)
{
	...
	case 0:
	{
		DfSetCaptureEngine(SparrowEngine::Normal);
	...
	case 2:
	{
		DfSetCaptureEngine(SparrowEngine::Black);
		processing_gui_settings_data_.Instance().engine = index;
	}
	break;

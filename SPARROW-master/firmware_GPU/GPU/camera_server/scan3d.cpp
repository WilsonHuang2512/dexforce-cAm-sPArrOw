...
Scan3D::~Scan3D()
{
    // [26-03-21] TODO: 似乎没有析构camera_ or lc3010_, 继承可能基类需要虚析构.
    //                  Scan3D类已知存在内存泄漏, 年前测试时发现的.
...
int Scan3D::init()
{
    ...
    {
        LOG(INFO) << "MCU Version: " << MCUVersion;
    }
...
int Scan3D::reopenCamera()
{
    ...
    LOG(INFO)<<"reopen Camera finished!";
    return DF_SUCCESS;    
}
...
{ 
    ...
    if (!camera_->setExposure(exposure))
    {
        ...
        LOG(ERROR) << "Camera setExposure failed";
        return false;
    }
...
int Scan3D::captureFrame06Mono12()
{
    ...
    { 
        ...
        std::cout << "bittttttt" << bits << std::endl;
        if(12 != bits)
        {
            if(setPixelFormat(12))
            {
...
bool Scan3D::loadCalibData()
{
    ...
    {  
        ...
        // [26-03-21] 增加baseline输出, 保持和CPU版本输出一致
        LOG(INFO) << "baseline: " << b;

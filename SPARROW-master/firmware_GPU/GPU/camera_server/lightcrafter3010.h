...
#define SingleChip_Write_Working_Mode                0xE2 // 0:普通；1:HDR
#define SingleChip_Write_Hdr_Data                    0xE3
#define SingleChip_Write_Min_Exposure                0xE6
#define reboot_2010                                  0xE7
#define SingleChip_Read_MCU_Version                  0xEA // 读取 MCU 版本号
#define SingleChip_Read_DLP_Status                   0xEB // 读取 DLP 状态
...
public:
    ...
    /**
     * @brief 设置HDR参数列表
     * @param [in] hdr_nums          HDR参数个数
     * @param [in] hdr_exposure_list HDR曝光参数列表
     * @param [in] hdr_current_list  HDR投影亮度(current电流)参数列表
     * @param [in] force             强制写入
     * @note [26-03-20] 增加`force`参数: 不与类内缓存数据判断, 用于重启后直接写入HDR参数
     */
    void set_hdr_list(int hdr_nums, std::vector<int> hdr_exposure_list, std::vector<int> hdr_current_list, bool force = false);

...
int main()
{
	...
	//创建相机
	SCamera* p_camera = (SCamera*)createSCamera();
	...
	if (0 == ret_code)
	{
		ret_code = p_camera->setParamCameraConfidence(2);
		if (0 != ret_code)
		{
		...
		ret_code = p_camera->setParamGenerateBrightness(1, 2500);
		if (0 != ret_code)
		{
		...
		if (true)
		{
			...
			//设置相机曝光时间（us）
			ret_code = p_camera->setParamCameraExposure(3500);
			if (0 != ret_code)
			{
		...
		else
		{
			...
			else
			{
				...
				//设置相机曝光时间（us）
				ret_code = p_camera->setParamCameraExposure(5000);
				if (0 != ret_code)
				{
					std::cout << "Set Camera Exposure Error!" << std::endl;
				}
				...
				p_camera->setCaptureEngine(SparrowEngine::Reflect);
		...
		if (0 == ret_code)
		{
			...
			{
				//获取亮度图数据
				ret_code = p_camera->getColorBrightnessData(color_brightness_data, SparrowColor::Rgb);
				if (0 == ret_code)
				{

...
bool Calibrate_Function::writeCalibTxt_45(cv::Mat camera_intrinsic, cv::Mat camera_distortion, cv::Mat projector_instrinsic,
	cv::Mat projector_distortion, cv::Mat s_r, cv::Mat s_t, std::string path)
{
	if (!camera_intrinsic.data || !camera_distortion.data || !projector_instrinsic.data || !projector_distortion.data || !s_r.data || !s_t.data)
	{
	...
	stream << s_t.at<double>(0, 0)/10 << "\n";
	stream << s_t.at<double>(1, 0)/10 << "\n";
	stream << s_t.at<double>(2, 0)/10 << "\n";
...
double Calibrate_Function::correctExtrinsics(std::vector<std::vector<cv::Point2f>> camera_points_list,
	std::vector<std::vector<cv::Point2f>> dlp_points_list, std::string in_path, std::string out_path, float* c_in, float* c_dis, float* p_in, float* p_dis, float* r, float* t)
{ 
	...
	cv::Mat rr;
	cv::Mat tt;
	s_camera_intrinsic.convertTo(c_intrinsic, CV_32F);
	s_camera_distortion.convertTo(c_distortion, CV_32F);
	s_project_intrinsic.convertTo(p_intrinsic, CV_32F);
	s_projector_distortion.convertTo(p_distortion, CV_32F);
	_R.convertTo(rr, CV_32F);
	cv::Mat TT=_T / 10;
	TT.convertTo(tt, CV_32F);
	if (c_in != nullptr) {
		std::memcpy(c_in, c_intrinsic.ptr<float>(0), c_intrinsic.total() * sizeof(float));
	}
	if (c_dis != nullptr) {
		std::memcpy(c_dis, c_distortion.ptr<float>(0), c_distortion.total() * sizeof(float));
	}
	if (p_in != nullptr) {
		std::memcpy(p_in, p_intrinsic.ptr<float>(0), p_intrinsic.total() * sizeof(float));
	}
	if (p_dis != nullptr) {
		std::memcpy(p_dis, p_distortion.ptr<float>(0), p_distortion.total() * sizeof(float));
	}
	if (r != nullptr) {
		std::memcpy(r, rr.ptr<float>(0), rr.total() * sizeof(float));
	}
	if (t != nullptr) {
		std::memcpy(t, tt.ptr<float>(0), tt.total() * sizeof(float));
	}
	...
	if (board_message_.width == 450) {

		std::cout << "R: " << "\n" << _R << "\n";
		std::cout << "T: " << "\n" << _T / 10 << "\n";
		ret = writeCalibTxt_45(s_camera_intrinsic, s_camera_distortion, s_project_intrinsic, s_projector_distortion, _R, _T, out_path);
	...
	else
	{
		std::cout << "R: " << "\n" << _R << "\n";
		std::cout << "T: " << "\n" << _T << "\n";
		ret = writeCalibTxt(s_camera_intrinsic, s_camera_distortion, s_project_intrinsic, s_projector_distortion, _R, _T, out_path);

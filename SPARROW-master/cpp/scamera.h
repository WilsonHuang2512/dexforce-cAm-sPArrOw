#pragma once
#ifndef __SPARROW_SCAMERA_H__
#define __SPARROW_SCAMERA_H__
#include<vector>
#include<string>
#include <functional>
#ifdef USE_OPENCV
...
#ifdef NEUTRAL_MODE
#define SparrowEngine Engine
#endif
		enum class SparrowEngine
		{
			Normal = 0,
			Reflect = 1,
			Black = 2,
...
#ifdef NEUTRAL_MODE
#define SparrowColor Color
#endif
		enum class SparrowColor
		{
			Rgb = 0,
			Bgr = 1,
			Bayer = 2,
        ...
        public:
            ...
            SCamera(const SCamera&) = delete;
            SCamera& operator=(const SCamera&) = delete;
			...
			virtual bool get_cameraSN(char SN[64]) = 0;
			...
			//输出参数：无
			//返回值：  类型（int）:返回0表示连接成功;返回-1表示连接失败.
			virtual int get_calib_param(const char* calib_param_path) = 0;
			...
			//输出参数：无
			//返回值：  类型（int）:返回0表示连接成功;返回-1表示连接失败.
			virtual int get_onecalibrationdata(const char* calib_param_path) = 0;
			...
			//输出参数：无
			//返回值：  类型（int）:返回0表示连接成功;返回-1表示连接失败.
			virtual int correct_stereo_Mono(int board_size, std::string patterns_path, std::string in_path, std::string out_path, float* c_in, float* c_dis, float* p_in, float* p_dis, float* r, float* t, double& error) = 0;
			...
			//输出参数：无
			//返回值：  类型（int）:返回0表示连接成功;返回-1表示连接失败.
			virtual int correct_stereo_Color(int board_size, std::string patterns_path, std::string in_path, std::string out_path, float* c_in, float* c_dis, float* p_in, float* p_dis, float* r, float* t, double& error) = 0;
			...
			//输出参数：无
			//返回值：  类型（int）:返回0表示连接成功;返回-1表示连接失败.
			virtual int set_calib_looktable(float* c_in, float* c_dis, float* p_in, float* p_dis, float* r, float* t) = 0;
#endif	
			//功能： 注册回调函数
			//输入参数： p_function（回调函数指针）
			//输出参数： 无
			//返回值： 类型（int）:返回0表示连接成功;返回-1表示连接失败.
			virtual int registerOnDropped(std::function<int(void*)> p_function) = 0;
			...
			virtual int setCaptureEngine(SparrowEngine engine) = 0;
			...
			//输出参数：engine
			//返回值： 类型（int）:返回0表示设置参数成功;返回-1表示设置参数失败。
			virtual int getCaptureEngine(SparrowEngine& engine) = 0;
			...
			//返回值： 类型（int）:返回0表示获取数据成功;返回-1表示采集数据失败.
			virtual int getColorBrightnessData(unsigned char* brightness, SparrowColor color) = 0;
			...
			//输出参数： brightness(亮度图)
			//返回值： 类型（int）:返回0表示获取数据成功;返回-1表示采集数据失败.
			virtual int getUndistortColorBrightnessData(unsigned char* brightness, SparrowColor color) = 0;
			...
			//输出参数： brightness(亮度图)
			//返回值： 类型（int）:返回0表示获取数据成功;返回-1表示采集数据失败.
			virtual int captureBrightnessData(unsigned char* brightness, SparrowColor color) = 0;
...
#ifdef NEUTRAL_MODE
		CAMERA_API void* createSCamera();
		CAMERA_API void destroySCamera(void*);
#else
		SPARROW_API void* createSCamera();
		SPARROW_API void destroySCamera(void*);
#endif

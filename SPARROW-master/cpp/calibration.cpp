...
\n\
1.Calibrate:\n\
sparrow_calibration.exe --calibrate --use patterns --path ./capture/calib --version 3010 --board 20 --calib ./param.txt \n\
\n\
2.Get Version:\n\
sparrow_calibration.exe --get-version \n\
\n\
3.Calibrate:\n\
sparrow_calibration.exe --calibrate --use phase --path ./capture/phase --version 3010 --board 20 --calib ./param.txt \n\
\n\
4.Calibrate:\n\
sparrow_calibration.exe --calibrate --use patterns-c --path ./capture/calib --version 3010 --board 20 --calib ./param.txt \n\
\n\
5.Correct:\n\
sparrow_calibration.exe --correct --use patterns --path ./capture/calib --version 3010 --board 20 --param-in ./param.txt --param-out ./param.txt \n\
\n\
6.Correct-c:\n\
sparrow_calibration.exe --correct --use patterns-c --path ./capture/calib --version 3010 --board 20 --param-in ./param.txt --param-out ./param.txt \n\
";
...
bool correct_stereo_color(std::string patterns_str, std::string param_in_str, std::string param_out_str);
...
//		if ("180" == board_str)
//		{
//			board_message.rows = 3;
//			board_message.cols = 2;
//			board_message.width = 180;
//			board_message.height = 90;
//		}
//
//		if ("130" == board_str)
//		{
//			board_message.rows = 3;
//			board_message.cols = 2;
//			board_message.width = 130;
//			board_message.height = 65;
//		}
//
//		if ("90" == board_str)
//		{
//			board_message.rows = 3;
//			board_message.cols = 2;
//			board_message.width = 90;
//			board_message.height = 45;
//		}
//
//		if ("45" == board_str)
//		{
//			board_message.rows = 3;
//			board_message.cols = 2;
//			board_message.width = 450;
//			board_message.height = 225;
//		}
//
//		if ("4" == board_str)
//		{
...
bool calibrate_stereo_base_phase(std::string phase_path, std::string calib_path)
{
	...
	for (int b_i = 0; b_i < current_folder_list.size(); b_i++)
	{
		...
		if (found)
		{
			...
			std::vector<std::string> str_list = vStringSplits(current_folder_list[b_i], "/");
...
bool calibrate_stereo_color(std::string patterns_path, std::string calib_path)
{
	...
	for (int g_i = 0; g_i < files_list.size(); g_i++)
	{
		...
		bool found;
#ifdef _WIN32 
		if (board_message.rows == 3) {
			std::cout << "screen:" << g_i << "img" << std::endl;
			found = detectCCTag(img, circle_points, id_list, status_list, g_i);
		}
		else
		{
			found = calib_function.findCircleBoardFeature(img, circle_points);
		}
#elif __linux
		found = calib_function.findCircleBoardFeature(img, circle_points);
#endif 
		...
		if (found)
		{
			...
			std::vector<std::string> str_list = vStringSplits(folder_list[g_i], "/");
...
bool correct_stereo_color(std::string patterns_path, std::string in_path, std::string out_path)
{
	...
	for (int g_i = 0; g_i < files_list.size(); g_i++)
	{
		...
		if (found)
		{
			...
			cv::resizeWindow("img", cv::Size(720, 480));
...
bool correct_stereo_monO(int board_size, std::string patterns_path, std::string in_path, std::string out_path, float* c_in, float* c_dis, float* p_in, float* p_dis, float* r, float* t, double& error) {
	...
	std::cout << "Camera Select Group Number:" << select_camera_points_list.size() << std::endl;
	if (select_dlp_points_list.size() == select_camera_points_list.size() && select_dlp_points_list.size() >= 4) {

		double stereo_err = calib_function.correctExtrinsics(select_camera_points_list, select_dlp_points_list, in_path, out_path, c_in, c_dis, p_in, p_dis, r, t);
		error = stereo_err;
		std::cout << "Reprojection Error: " << stereo_err << std::endl;
		std::cout << "Reprojection Error should be less than 0.1......" << std::endl;
	}

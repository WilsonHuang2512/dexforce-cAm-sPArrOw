...
double Calibrate_Function::calibrateStereo(std::vector<std::vector<cv::Point2f>> camera_points_list, std::vector<std::vector<cv::Point2f>> dlp_points_list, std::string path)
{
	...
	if (board_message_.width == 450) {
		...
		std::cout << "R: " << "\n" << _R << "\n";
		std::cout << "T: " << "\n" << _T / 10 << "\n";
		ret = writeCalibTxt_45(s_camera_intrinsic, s_camera_distortion, s_project_intrinsic, s_projector_distortion, _R, _T, path);
	...
	else
	{
		std::cout << "R: " << "\n" << _R << "\n";
		std::cout << "T: " << "\n" << _T  << "\n";
		ret = writeCalibTxt(s_camera_intrinsic, s_camera_distortion, s_project_intrinsic, s_projector_distortion, _R, _T, path);
	}
...
std::vector<cv::Point3f> Calibrate_Function::generateAsymmetricWorldFeature_45(struct BoardMessage board_message)
{
	std::vector<cv::Point3f> objectCorners;
	for (int r = 0; r < board_message.rows; r++)
	{
		for (int c = 0; c < board_message.cols; c++)
		{
			...
			if (0 == r % 2)
			{
				...
				objectCorners.push_back(cv::Point3f((board_message.width * c)/10,(board_message.height * r)/10, 0.0f));
			}
			else if (1 == r % 2)
			{

				objectCorners.push_back(cv::Point3f((board_message.width * c + 0.5 * board_message.width)/10, (board_message.height * r)/10, 0.0f));
			}

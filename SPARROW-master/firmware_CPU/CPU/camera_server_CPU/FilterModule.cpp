...
bool FilterModule::statisticOutlierRemoval_cpu(int height,int width,cv::Mat& point_cloud_map,cv::Mat& depth, int num_neighbors, double threshold_radio)
{
	if (!point_cloud_map.data)
		return false;
...
#pragma omp parallel for reduction(+:num) 
	...
	for (int r = 0; r < nr; r++)
	{
		...
		double* ptr_depth = depth.ptr<double>(r);
		cv::Vec3d* ptr_p = point_cloud_map.ptr<cv::Vec3d>(r);
		double* ptr_mean = mean_map.ptr<double>(r);
...
bool FilterModule::monotonicityFilter_cpu(cv::Mat& unwrapMap, cv::Mat& maskOutput, float monotonicityThresholdVal, float monotonicityFilterVal) {


	monotonicityFilterVal = monotonicityFilterVal / 100. - 0.5;
	CV_Assert(unwrapMap.type() == CV_32FC1);

	maskOutput = cv::Mat::zeros(unwrapMap.size(), CV_8UC1);

	int imgHeight = unwrapMap.rows;
	int imgWidth = unwrapMap.cols;

	for (int idy = 0; idy < imgHeight; ++idy) {
		for (int idx = 1; idx < imgWidth - 1; ++idx) { 
			float beforePhase = unwrapMap.at<float>(idy, idx - 1);
			float thisPhase = unwrapMap.at<float>(idy, idx);
			float nextPhase = unwrapMap.at<float>(idy, idx + 1);
			float diff1 = thisPhase - beforePhase;
			float diff2 = nextPhase - thisPhase;

			if (thisPhase < 0.1 || (beforePhase < 0.1 && nextPhase < 0.1)) {
				maskOutput.at<unsigned char>(idy, idx) = 255;
				continue;
			}

			if (beforePhase >= 0.1) {
				if (diff1 > monotonicityThresholdVal && diff1 < monotonicityFilterVal) {
					maskOutput.at<unsigned char>(idy, idx) = 0;
					continue;
				}
			}

			if (nextPhase >= 0.1) {
				if (diff2 > monotonicityThresholdVal && diff2 < monotonicityFilterVal) {
					maskOutput.at<unsigned char>(idy, idx) = 0;
					continue;
				}
			}
	...
	for (int y = 0; y < imgHeight; ++y) {
		for (int x = 0; x < imgWidth; ++x) {
			if (maskOutput.at<uchar>(y, x) == 0) {
				unwrapMap.at<float>(y, x) = 0;
			}
		}
	}
}

void FilterModule::depth_filter_step_1( cv::Mat& depth_map, cv::Mat& depth_map_temp, cv::Mat& mask_temp, float depth_threshold) {

	CV_Assert(depth_map.type() == CV_64FC1);   

	int img_height = depth_map.rows;
	int img_width = depth_map.cols;
	...
	for (int y = 1; y < img_height - 1; y++) {
		for (int x = 1; x < img_width; x++) {
			...
			if (depth <= 0) {
				depth_map_temp.at<double>(y, x) = -1;
				continue;
			}
			...
			for (int dy = -1; dy <= 1; dy++) {
				for (int dx = -1; dx <= 1; dx++) {
					...
					double neighbour_depth = depth_map.at<double>(y + dy, x + dx);
					if (neighbour_depth > 0) {
						depth_diff[count++] = std::abs(neighbour_depth - depth) * 2.0f / (neighbour_depth + depth);
					}
					else {
						depth_diff[count++] = -1;
					}
			...
			depth_map_temp.at<double>(y, x) = std::abs(maxDepthDiff);

			if (std::abs(maxDepthDiff) > depth_threshold) {
				mask_temp.at<unsigned char>(y, x) = 0;
			}
		}
	}
}
void FilterModule::depth_filter_step_2(cv::Mat& depth_map, const cv::Mat& depth_map_temp, cv::Mat& mask_temp, float depth_threshold) {
	...
	for (int y = 2; y < img_height - 2; ++y) {
		for (int x = 0; x < img_width; ++x) {
			if (mask_temp.at<unsigned char>(y, x) == 255) {
				mask_temp.at<unsigned char>(y, x) = 0;
				continue;
			}

			std::vector<double> depthDiff(8, -1);
		 
			if (x > 0 && x < img_width - 1) {
				depthDiff[0] = depth_map_temp.at<double>(y - 1, x - 1);
				depthDiff[1] = depth_map_temp.at<double>(y - 1, x);
				depthDiff[2] = depth_map_temp.at<double>(y - 1, x + 1);
				depthDiff[3] = depth_map_temp.at<double>(y, x - 1);
				depthDiff[4] = depth_map_temp.at<double>(y, x + 1);
				depthDiff[5] = depth_map_temp.at<double>(y + 1, x - 1);
				depthDiff[6] = depth_map_temp.at<double>(y + 1, x);
				depthDiff[7] = depth_map_temp.at<double>(y + 1, x + 1);
			}

	
			depthDiff.erase(std::remove(depthDiff.begin(), depthDiff.end(), -1), depthDiff.end());
			if (!depthDiff.empty()) {
				std::nth_element(depthDiff.begin(), depthDiff.begin() + depthDiff.size() / 2, depthDiff.end());
				float depthFeatureResult = depthDiff[depthDiff.size() / 2]; 
...
bool FilterModule::RadiusOutlierRemoval_cpu(int height,int width,cv::Mat& point_cloud_map,cv::Mat& depth, cv::Mat& mask, double dot_spacing, double radius, int points_num)
{
	if (!point_cloud_map.data)
		return false;
	...
	//int rest_num = findNearPointsnum(point_cloud_map, mask, cv::Point(808, 630), radius);
	//#pragma omp parallel for
	//cv::Mat disparity_map = cv::Mat(height, width, CV_64F, cv::Scalar(0));
...
int FilterModule::findNearPointsnum(cv::Mat& point_cloud_map, cv::Mat& mask, cv::Point pos, double dot_spacing, double radius)
{
	...
	//std::cout << "e_r-s_r" << e_r - e_c << std::endl;
	//std::cout << "e_c-s_c" << e_c - s_c << std::endl;
...
double FilterModule::computePointsDistance(cv::Vec3d p0, cv::Vec3d p1)
{
	...
	double val = p_d[0] * p_d[0] + p_d[1] * p_d[1] + p_d[2] * p_d[2];

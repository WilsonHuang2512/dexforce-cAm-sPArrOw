...
bool DF_Encode::unwrapBase2Kmap_black(cv::Mat wrap_map, cv::Mat k2_map, cv::Mat& unwrap_map)
{
	if (wrap_map.empty() || k2_map.empty())
	{
		return false;
...
bool DF_Encode::computePhaseShift_repetition_black(std::vector<cv::Mat> patterns, cv::Mat& wrap_map, cv::Mat& mask_map)
{
	if (patterns.empty())
	{
		return false;
...
bool DF_Encode::decodeMinswGrayCode_black(cv::Mat patterns, int space, cv::Mat average_brightness, cv::Mat& k_map)
{
	...
	for (int r = 0; r < nr; r++)
	{
		...
		ushort* ptr_gray = patterns.ptr<ushort>(r);
		uchar* ptr_val = k_map.ptr<uchar>(r);

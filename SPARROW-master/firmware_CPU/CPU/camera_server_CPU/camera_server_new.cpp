...
{
    // 重启相机
    LOG(INFO) << "reboot camera";
    scan3d_.reopenCamera();

    LOG(INFO) << "reboot lightcraft";
    lc3010->off_reboot_2010();
    std::this_thread::sleep_for(std::chrono::seconds(2));
    // 通过 MCU 查询 DLP 启动状态
    LOG(INFO) << "Reading DLP Status...";
    auto camearPtr = scan3d_.getCameraDevice();
    bool dlpStatus = false;
    if (camearPtr->getDLPStatus(dlpStatus))
    {
        ...
        if (!dlpStatus)
        {
            for (int i = 1; i <= 30; ++i)
            {
                ...
                std::this_thread::sleep_for(std::chrono::seconds(2));
                camearPtr->getDLPStatus(dlpStatus);
                LOG(INFO) << "DLP Status: " << dlpStatus;
                // 光机启动则跳出等待, 继续执行其他任务
                if (dlpStatus) break;
            }
    ...
    while (operate_num-- > 0)
    {
        ...
        if (DF_SUCCESS != ret)
        {
            LOG(ERROR) << "lc2010 SetLedCurrent FAILED; CODE : " << ret;
            //usleep(100000);
    ...
    // [26-03-20] 恢复曝光时间和HDR参数列表等
    scan3d_.recoverParam();

    return 0;
}

int recapture(const std::function<int()>& captureFunc)
{
    // 等待一段时间再次采集
    ...
    if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret)
    {
        ...
        {
            LOG(INFO) << "Recapture [" << i << "/" << retryTimes << "]";
            ret = captureFunc();
            if (DF_SUCCESS == ret)
            {
                LOG(INFO) << "Recapture SUCCESS";
                return ret;
            }

            LOG(INFO) << "Recapture FAILED (retry " << i << ")";
            LOG(INFO) << "STATUS: " << ret;
...
int handle_cmd_get_frame_06_repetition_black_cpu(int client_sock)
{
    /**************************************************************************************/
    ...
    ret = scan3d_.captureFrame06RepetitionMono12_cpu(repetition_count);
    ...
    if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret) {
        ...
        ret = recapture([&]() {
            return scan3d_.captureFrame06RepetitionMono12_cpu(repetition_count);
            });
        if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret)
            return DF_FAILED;
...
int handle_cmd_get_frame_06_repetition_color(int client_sock)
{
    ...
    ret = scan3d_.captureFrame06RepetitionColor_cpu(repetition_count);
    ...
    if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret) {
        ...
        ret = recapture([&]() {
            return scan3d_.captureFrame06RepetitionColor_cpu(repetition_count);
            });
        if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret)
            return DF_FAILED;
...
int handle_cmd_get_frame_06_hdr_color(int client_sock)
{
    ...
    int ret = scan3d_.captureFrame06HdrColor_cpu(); 
    ...
    if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret) {
        ...
        LOG(INFO) << "The second collection begins";
        ret = recapture([&]() {
            return scan3d_.captureFrame06HdrColor_cpu();
            });
        if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret)
            return DF_FAILED;
        //reboot_lightcraft();
        //ret = scan3d_.captureFrame06HdrColor_cpu();
    }
...
int handle_cmd_get_frame_06_black_cpu(int client_sock)
{
    if (check_token(client_sock) == DF_FAILED)
    {
        return DF_FAILED;
    ...
    int ret = scan3d_.captureFrame06Mono12_cpu();
    ...
    if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret) {
        ...
        ret = recapture([&]() {
            return scan3d_.captureFrame06Mono12_cpu();
            });
        if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret)
            return DF_FAILED;

        //reboot_lightcraft();
        //ret = scan3d_.captureFrame06Mono12_cpu();
    }
...
int handle_cmd_get_frame_06_hdr_black_cpu(int client_sock)
{
    if (check_token(client_sock) == DF_FAILED)
    {
        return DF_FAILED;
    ...
    int ret = scan3d_.captureFrame06HdrMono12_cpu();
    if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret) {
        ...
        ret = recapture([&]() {
            return scan3d_.captureFrame06HdrMono12_cpu();
            });
        if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret)
            return DF_FAILED;

        //reboot_lightcraft();
        //ret = scan3d_.captureFrame06HdrMono12_cpu();
    }
...
int handle_cmd_get_frame_06_hdr_cpu(int client_sock)
{
    ...
    if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret) {
        ...
        ret = recapture([&]() {
            return scan3d_.captureFrame06Hdr_cpu();
            });
        if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret)
            return DF_FAILED;
...
int handle_cmd_get_frame_06_repetition_cpu(int client_sock)
{
    ...
    if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret) {
        ...
        ret = recapture([&]() {
            return scan3d_.captureFrame06Repetition_cpu(repetition_count);
            });
        if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret)
            return DF_FAILED;
...
int handle_cmd_get_frame_06_color(int client_sock)
{
    ...
    ret = scan3d_.captureFrame06Color_cpu();
    ...
    if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret) {
        ...
        ret = recapture([&]() {
            return scan3d_.captureFrame06Color_cpu();
            });
        if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret)
            return DF_FAILED;
...
int handle_cmd_get_frame_06_cpu(int client_sock)
{
    ...
    if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret) {
        ...
        ret = recapture([&]() {
            return scan3d_.captureFrame06_cpu();
            });
        if (DF_ERROR_CAMERA_GRAP == ret || DF_ERROR_LOST_TRIGGER == ret)
            return DF_FAILED;
...
int handle_cmd_set_calib_params(int client_sock)
{
    if (check_token(client_sock) == DF_FAILED)
    {
        return DF_FAILED;
    ...
    float calib_param[40];

    int ret = recv_buffer(client_sock, (char*)(calib_param), sizeof(float) * 40);
    if (ret == DF_FAILED)
    {
        LOG(INFO) << "send error, close this connection!\n";
        return DF_FAILED;
    ...
    std::cout << "recieve_calib_param: ";
    for (int i = 0; i < 40; ++i) {
        std::cout << calib_param[i] << std::endl;;
    }


    scan3d_.set_calib_param(calib_param);
...
int handle_get_firmware_version(int client_sock)
{
   ...
   char version[_VERSION_LENGTH_] = _VERSION_NUMBER_;
...
int handle_commands(int client_sock)
{
    ...
    case DF_CMD_GET_FRAME_06_MONO12:
        {
            ...
            if (type == XemaPixelType::Mono)
            {
                ret = handle_cmd_get_frame_06_black_cpu(client_sock);
            }
            else if (type == XemaPixelType::BayerRG8)
            {
                ret = handle_cmd_get_frame_06_color(client_sock);
    ...
    { 
        LOG(INFO) << "DF_CMD_GET_FRAME_06_MONO12";
            ...
            if (type == XemaPixelType::Mono)
            {
                ret = handle_cmd_get_frame_06_hdr_black_cpu(client_sock);
            }
            else if (type == XemaPixelType::BayerRG8)
            {
                ret = handle_cmd_get_frame_06_hdr_color(client_sock);
    ...
    case DF_CMD_GET_REPETITION_FRAME_06_MONO12:
    {
            ...
            if (type == XemaPixelType::Mono)
            { 
                ret = handle_cmd_get_frame_06_repetition_black_cpu(client_sock); 
    ...
    case DF_CMD_SET_CAMERA_LOOKTABLE:
        ...
        read_calib_param();
        break;
    case DF_CMD_SET_CALIB_PARAM:
        LOG(INFO) << "DF_CMD_SET_CALIB_PARAM";
        ret = handle_cmd_set_calib_params(client_sock);
        break;
...
{  
    ...
    // [26-03-27] 修正光机型号, 为避免直接修改 version 导致工程其他地方异常, 此处直接打印2010
    LOG(INFO) << "read camera version: " << 2010;

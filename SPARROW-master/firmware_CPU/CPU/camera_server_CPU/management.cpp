...
int d_image_width_ = 0;
int d_image_height_ = 0;
/**********************************************************************/
//basic memory
unsigned char* d_patterns_list_[MAX_PATTERNS_NUMBER];
unsigned char* d_six_step_patterns_list_[LAST_STEPS_NUM];
unsigned char* d_six_step_patterns_convolved_list_[LAST_STEPS_NUM];
unsigned short* d_repetition_02_merge_patterns_convolved_list_[LAST_STEPS_NUM];
float* d_wrap_map_list_[MAX_WRAP_NUMBER];
float* d_confidence_map_list_[MAX_WRAP_NUMBER];
float* d_unwrap_map_list_[MAX_UNWRAP_NUMBER];
float* d_six_step_pattern_convolution_phase_list_[2];

float* d_fisher_confidence_map;
float* d_convolution_kernal_map;
unsigned char* d_fisher_mask_;
unsigned char* d_mask_map_;
unsigned char* d_brightness_map_;
unsigned short* d_brightness_short_map_;
unsigned char* d_darkness_map_;
float* d_point_cloud_map_;
float* d_depth_map_;
float* d_depth_map_temp_;
float* d_triangulation_error_map_;
unsigned char* d_global_light_map_;
unsigned char* d_direct_light_map_;
unsigned char* d_uncertain_map_;

int load_calib_data_flag_ = 0;

float* d_camera_intrinsic_ = NULL;
float* d_project_intrinsic_ = NULL;
float* d_camera_distortion_ = NULL;
float* d_projector_distortion_ = NULL;
float* d_rotation_matrix_ = NULL;
float* d_translation_matrix_ = NULL;

unsigned char* d_minsw8_table_ = NULL;


float d_baseline_ = 0;
float* d_single_pattern_mapping_ = NULL;
float* d_single_pattern_minimapping_ = NULL;
float* d_xL_rotate_x_ = NULL;
float* d_xL_rotate_y_ = NULL;
float* d_R_1_ = NULL;

float* d_undistort_map_x_ = NULL;
float* d_undistort_map_y_ = NULL;
/**********************************************************************/
//hdr memory
...
unsigned short* d_repetition_02_merge_patterns_list_[D_REPETITION_02_MAX_NUM];
unsigned short* d_merge_brightness_map_;
//start
...
{
	int image_size = d_image_height_ * d_image_width_;

	for (int i = 0; i < MAX_PATTERNS_NUMBER; i++)
	{
		d_patterns_list_[i] = new unsigned char[image_size];
	}
	for (int i = 0; i < LAST_STEPS_NUM; i++)
	{
		d_six_step_patterns_list_[i] = new unsigned char[image_size];
	}
	for (int i = 0; i < LAST_STEPS_NUM; i++)
	{
		d_six_step_patterns_convolved_list_[i] = new unsigned char[image_size];
	}
	for (int i = 0; i < LAST_STEPS_NUM; i++)
	{
		d_repetition_02_merge_patterns_convolved_list_[i] = new unsigned short[image_size];
	}


	for (int i = 0; i< MAX_WRAP_NUMBER; i++)
	{
		d_wrap_map_list_[i]=new float[image_size];
		d_confidence_map_list_[i]=new float[image_size];
	}

	for (int i = 0; i< MAX_UNWRAP_NUMBER; i++)
	{
		d_unwrap_map_list_[i]=new float[image_size]; 
	} 

	for (int i = 0; i< 2; i++)
	{
		d_six_step_pattern_convolution_phase_list_[i]=new float[image_size]; 
	} 

	d_brightness_map_=new unsigned char[image_size];
	d_brightness_short_map_=new unsigned short[image_size];  

	d_mask_map_ = new unsigned char[image_size];
	d_fisher_mask_ = new unsigned char[image_size];


	d_camera_intrinsic_=new float[9];
	d_project_intrinsic_ = new float[9];

	d_camera_distortion_ = new float[5];
	d_projector_distortion_ = new float[5];

	d_rotation_matrix_=new float[9];
	d_translation_matrix_ = new float[9];
 
	d_minsw8_table_=new unsigned char[256];
	...
	memcpy(d_minsw8_table_, array, 256 * sizeof(unsigned char));

	d_fisher_confidence_map = new float[d_image_height_ * d_image_width_];
	d_convolution_kernal_map = new float[SIZE_OF_CONVOLUTION_KERNAL * SIZE_OF_CONVOLUTION_KERNAL];
	d_point_cloud_map_ = new float[3 * d_image_height_ * d_image_width_];
	d_depth_map_ = new float[d_image_height_ * d_image_width_];
	d_depth_map_temp_ = new float[d_image_height_ * d_image_width_];
	d_triangulation_error_map_ = new float[d_image_height_ * d_image_width_];

	d_global_light_map_ = new unsigned char[d_image_height_ * d_image_width_];
	d_direct_light_map_ = new unsigned char[d_image_height_ * d_image_width_];
	d_uncertain_map_ = new unsigned char[d_image_height_ * d_image_width_];

	d_single_pattern_mapping_ = new float[4000 * 2000];
	d_single_pattern_minimapping_ = new float[128 * 128];
	d_xL_rotate_x_ = new float[d_image_height_ * d_image_width_];
	d_xL_rotate_y_ = new float[d_image_height_ * d_image_width_];
	d_R_1_ = new float[3 * 3];

	d_undistort_map_x_ = new float[d_image_height_ * d_image_width_];
	d_undistort_map_y_ = new float[d_image_height_ * d_image_width_];
...
bool cuda_free_basic_memory()
{
	...
	for (int i = 0; i< MAX_PATTERNS_NUMBER; i++)
	{  
		delete[] d_patterns_list_[i]; 
	}
	for (int i = 0; i< LAST_STEPS_NUM; i++)
	{  
		delete[] d_six_step_patterns_list_[i];
	}
	for (int i = 0; i< LAST_STEPS_NUM; i++)
	{  
		delete[] d_six_step_patterns_convolved_list_[i];
	}
	for (int i = 0; i< LAST_STEPS_NUM; i++)
	{  
		delete[] d_repetition_02_merge_patterns_convolved_list_[i];
	}


	for (int i = 0; i< MAX_WRAP_NUMBER; i++)
	{  
		delete[] d_wrap_map_list_[i];
		delete[] d_confidence_map_list_[i];
	}

	for (int i = 0; i< MAX_UNWRAP_NUMBER; i++)
	{ 
		delete[] d_unwrap_map_list_[i];
	}

	for (int i = 0; i< 2; i++)
	{ 
		delete[] d_six_step_pattern_convolution_phase_list_[i];
	}

	delete[] d_fisher_confidence_map;
	delete[] d_convolution_kernal_map;
	delete[] d_fisher_mask_;
	delete[] d_mask_map_;
	delete[] d_brightness_map_;
	delete[] d_brightness_short_map_;

	delete[] d_point_cloud_map_;
	delete[] d_depth_map_;
	delete[] d_depth_map_temp_;
	delete[] d_triangulation_error_map_;

	delete[] d_global_light_map_;
	delete[] d_direct_light_map_;
	delete[] d_uncertain_map_;
	


	delete[] d_camera_intrinsic_;
	delete[] d_project_intrinsic_;
	delete[] d_camera_distortion_;
	delete[] d_projector_distortion_;
	delete[] d_rotation_matrix_;
	delete[] d_translation_matrix_;

	delete[] d_minsw8_table_;
	
 
	delete[] d_single_pattern_mapping_;
	delete[] d_single_pattern_minimapping_;
	delete[] d_xL_rotate_x_;
	delete[] d_xL_rotate_y_;
	delete[] d_R_1_;

	delete[] d_undistort_map_x_;
	delete[] d_undistort_map_y_;
...
bool cuda_malloc_hdr_memory()
{
	...
	{
		d_hdr_depth_map_list_[i]=new float[d_image_height_ * d_image_width_];
		d_hdr_brightness_list_[i]=new unsigned char[d_image_height_*d_image_width_] ;  
		d_hdr_bright_pixel_sum_list_[i]=new float[1]; 
	}
	//cudaDeviceSynchronize();
	return true;
}
...
bool cuda_free_hdr_memory()
{
    for (int i = 0; i< D_HDR_MAX_NUM; i++)
	{ 
		delete[] d_hdr_depth_map_list_[i];
		delete[] d_hdr_brightness_list_[i]; 
		delete[] d_hdr_bright_pixel_sum_list_[i];
	}
...
bool cuda_malloc_repetition_memory()
{
	...
	{
		d_repetition_patterns_list_[i]=new unsigned char[d_image_height_*d_image_width_]; 
	}

	for(int i= 0;i< 6;i++)
	{
		d_repetition_merge_patterns_list_[i]=new unsigned short[d_image_height_*d_image_width_]; 
	}
 
 	for(int i= 0;i< D_REPETITION_02_MAX_NUM;i++)
	{
		d_repetition_02_merge_patterns_list_[i] = new unsigned short[d_image_height_*d_image_width_];
	}

	d_merge_brightness_map_ = new unsigned short[d_image_height_*d_image_width_];
...
bool cuda_free_repetition_memory()
{
	...
	for(int i= 0;i< D_REPETITIONB_MAX_NUM*6;i++)
	{
		delete[] d_repetition_patterns_list_[i]; 
	}

	for(int i= 0;i< 6;i++)
	{
		delete[] d_repetition_merge_patterns_list_[i];  
	}

	for(int i= 0;i< D_REPETITION_02_MAX_NUM;i++)
	{
		delete[] d_repetition_02_merge_patterns_list_[i];
	}

	delete[] d_merge_brightness_map_;
	...
	float* projector_distortion, float* rotation_matrix, float* translation_matrix)
{
	...
	memcpy(d_camera_intrinsic_, camera_intrinsic, 3 * 3 * sizeof(float));
	memcpy(d_project_intrinsic_, project_intrinsic, 3 * 3 * sizeof(float));

	memcpy(d_camera_distortion_, camera_distortion, 1 * 5 * sizeof(float));
	memcpy(d_projector_distortion_, projector_distortion, 1 * 5 * sizeof(float));

	memcpy(d_rotation_matrix_, rotation_matrix, 3 * 3 * sizeof(float));
	memcpy(d_translation_matrix_, translation_matrix, 1 * 3 * sizeof(float));
...
{
	memcpy(d_R_1_, r_1, 3 * 3 * sizeof(float));
	memcpy(d_single_pattern_minimapping_, mini_mapping, 128 * 128 * sizeof(float));
	memcpy(d_single_pattern_mapping_, mapping, 4000*2000 * sizeof(float));
	memcpy(d_xL_rotate_x_, rotate_x, d_image_height_*d_image_width_ * sizeof(float));
	memcpy(d_xL_rotate_y_, rotate_y, d_image_height_*d_image_width_ * sizeof(float));
...
void coud_copy_undistort_table_to_memory(float* undistort_x_map,float* undistort_y_map)
{
	...
	memcpy(d_undistort_map_x_, undistort_x_map, d_image_height_*d_image_width_ * sizeof(float));
	memcpy(d_undistort_map_y_, undistort_y_map, d_image_height_*d_image_width_ * sizeof(float));
}
...
bool cuda_copy_pattern_to_memory(unsigned char* pattern_ptr,int serial_flag)
{
	...
	memcpy(d_patterns_list_[serial_flag], smooth_mat.data, d_image_height_*d_image_width_* sizeof(unsigned char));
}

void cuda_copy_pointcloud_from_memory(float* pointcloud)
{ 
	memcpy(pointcloud, d_point_cloud_map_, 3 * d_image_height_*d_image_width_ * sizeof(float));
}

void cuda_copy_depth_from_memory(float* depth)
{
	memcpy(depth, d_depth_map_, d_image_height_*d_image_width_ * sizeof(float));
} 

void cuda_copy_brightness_from_memory(unsigned char* brightness)
{
	memcpy(brightness, d_brightness_map_, d_image_height_*d_image_width_ * sizeof(unsigned char));
}

void cuda_copy_convolution_kernal_to_memory(float* convolution_kernal, int kernal_diameter)
{
	memcpy(d_convolution_kernal_map, convolution_kernal, kernal_diameter*kernal_diameter* sizeof(float));
}


void cuda_copy_brightness_16_to_memory(unsigned short* brightness)
{
	memcpy(d_brightness_short_map_, brightness, d_image_height_*d_image_width_* sizeof(unsigned short));

}

void cuda_copy_brightness_to_memory(unsigned char* brightness)
{ 
	memcpy(d_brightness_map_, brightness, d_image_height_*d_image_width_* sizeof(unsigned char));
}
...
void cuda_clear_reconstruct_cache()
{
	...
	// 使用memset或std::fill_n来清零内存  
	memset(d_brightness_map_, 0, image_size * sizeof(char));
	memset(d_depth_map_, 0, image_size * sizeof(float));
	memset(d_point_cloud_map_, 0, 3 * image_size * sizeof(float));
	memset(d_mask_map_, 0, image_size * sizeof(unsigned char));
	memset(d_fisher_confidence_map, 0, image_size * sizeof(float));

	for (int i = 0; i < 6; ++i) {
		memset(d_hdr_brightness_list_[i], 0, image_size * sizeof(char));
	}

	// 根据条件决定是否清零特定的内存块  
	if (1 == cuda_system_config_settings_machine_.Instance().firwmare_param_.use_global_light_filter) {
		memset(d_direct_light_map_, 0, image_size * sizeof(char));
		memset(d_global_light_map_, 0, image_size * sizeof(char));
		memset(d_uncertain_map_, 0, image_size * sizeof(char));
	}
}
...
bool cuda_merge_brigntness(int hdr_num, unsigned char* brightness)
{
	...
	for (int i = 0; i < hdr_num; i++)
	{
		...
		memcpy(image_b.data, d_hdr_brightness_list_[i], 1 * h_image_height_ * h_image_width_ * sizeof(uchar));
...
bool cuda_clear_repetition_02_patterns()
{
	for(int i = 0;i< D_REPETITION_02_MAX_NUM;i++)
	{
		memset(d_repetition_02_merge_patterns_list_[i], 0, h_image_height_ * h_image_width_ * sizeof(ushort));

	}
	memset(d_merge_brightness_map_, 0, h_image_height_ * h_image_width_ * sizeof(ushort));
	memset(d_brightness_short_map_, 0, h_image_height_ * h_image_width_ * sizeof(ushort));
	...
	std::vector<cv::Mat>& repetition_02_merge_patterns_list)
{
	...
	if (0 == repetition_serial)
	{
		for (int i = 0; i < nr; i++) {
			...
			ushort* patterns0 = patterns_[repetition_serial].ptr<ushort>(i);
			ushort* merge_bright = merge_brightness_map.ptr<ushort>(i);
			for (int j = 0; j < nc; j++) {
				merge_bright[j] += patterns0[j];
	...
	for (int i = 0; i < nr; i++) {
		...
		ushort* patterns = patterns_[repetition_serial].ptr<ushort>(i);
		ushort* repetition_02_merge_patterns = repetition_02_merge_patterns_list[repetition_serial].ptr<ushort>(i);
		for (int j = 0; j < nc; j++) {
			repetition_02_merge_patterns[j] += patterns[j];
		}
...
{
	cv::Mat img = img_ptr.clone();
	cv::Mat brightness = brightness_map.clone();

	for (int i = 0; i < nr; i++) {
		unsigned short* img_ptrs = img.ptr<unsigned short>(i);
		unsigned char* brightness_map_ptr = brightness.ptr<unsigned char>(i);
		for (int j = 0; j < nc; j++) {
			brightness_map_ptr[j]= 0.5 + (img_ptrs[j]/16);
		}
	}
...
int cuda_handle_minsw8_16_cpu(int nr, int nc, int flag, std::vector<cv::Mat>& patterns_, cv::Mat& threshold_map, cv::Mat& mask, cv::Mat& wrap, cv::Mat& sw_k_map, cv::Mat& minsw_map, cv::Mat& k2_map, cv::Mat& unwrap)
{
	...
	case 8:
	{
		...
		bool ret = encode.computePhaseShift_repetition_black(phase_shift_patterns_img, wrap, mask);
	...
	if (flag > 7 && flag < 16)
	{
		...
		//minsw数据8张
		int space = flag - 8;
		bool ret1 = encode.decodeMinswGrayCode_black(patterns_[flag], space, threshold_map, sw_k_map);
	}
	...
	if (15 == flag)
	{
		...
		encode.unwrapBase2Kmap_black(wrap, k2_map, unwrap);

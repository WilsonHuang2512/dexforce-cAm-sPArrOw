...
Scan3D::Scan3D() :
    buff_brightness_(nullptr),
    buff_depth_(nullptr),
    buff_pointcloud_(nullptr),
    camera_(nullptr),
    lc3010_(nullptr)
{
...
Scan3D::~Scan3D()
{
    // [26-03-26] 对齐GPU版本逻辑, 释放资源
    if (this->buff_brightness_ != nullptr)
    {
        delete[] buff_brightness_;
        this->buff_brightness_ = nullptr;
    }
    if (this->buff_depth_ != nullptr)
    {
        delete[] buff_depth_;
        this->buff_depth_ = nullptr;
    }
    if (this->buff_pointcloud_ != nullptr)
    {
        delete[] buff_pointcloud_;
        this->buff_pointcloud_ = nullptr;
    }
}
...
int Scan3D::init()
{
    ...
    // 读取MCU版本号
    LOG(INFO) << "Reading MCU Version...";
    std::string MCUVersion;
    //auto start =  std::chrono::system_clock::now();
    if (camera_->getMCUVersion(MCUVersion))
    {
        LOG(INFO) << "MCU " << MCUVersion;
    }
    ...
    {
        LOG(INFO) << "MCU Version not support.";
    }
    ...
    // 通过 MCU 查询 DLP 启动状态
    LOG(INFO) << "Reading DLP Status...";
    bool dlpStatus = false;
    if (camera_->getDLPStatus(dlpStatus))
    {
        LOG(INFO) << "DLP Status: " << dlpStatus;
        // 未启动则等待继续查询
        if (!dlpStatus)
        {
            for (int i = 1; i <= 30; ++i)
            {
                LOG(INFO) << "Waiting DLP open...";
                std::this_thread::sleep_for(std::chrono::seconds(2));
                camera_->getDLPStatus(dlpStatus);
                LOG(INFO) << "DLP Status: " << dlpStatus;
                // 光机启动则跳出等待, 继续执行其他任务
                if (dlpStatus) break;
            }
    ...
    {
        LOG(INFO) << "DLP Status not support.";
    }
    ...
    if (!dlpStatus)
    {
        LOG(INFO) << "DLP Status is OFF.";
        // 为兼容旧版 MCU 代码, 不返回初始化失败
        // return DF_FAILED;
    }
    ...
    if (DF_SUCCESS != ret)
    {
        LOG(ERROR) << "lc2010 init FAILED; CODE : " << ret;
    }
...
int Scan3D::set_calib_param(float* param) {

    camera_->set_calib_param(param);
    return 0;
}

void Scan3D::bufferMemset()
{
    if (buff_brightness_ != nullptr && image_width_ > 0 && image_height_ > 0)
    {
        memset(buff_brightness_, 0, image_width_ * image_height_ * sizeof(unsigned char));
    }
    if (buff_depth_ != nullptr && image_width_ > 0 && image_height_ > 0)
    {
        memset(buff_depth_, 0, image_width_ * image_height_ * sizeof(float));
    }
    if (buff_pointcloud_ != nullptr && image_width_ > 0 && image_height_ > 0)
    {
        memset(buff_pointcloud_, 0, 3 * image_width_ * image_height_ * sizeof(float));
    }
}

void Scan3D::recoverParam()
{
    bufferMemset();
    recoverParamHdr();
    recoverParamExposure();
    recoverParamGain();
    recoverParamLedCurrent();
}
...
bool Scan3D::recoverParamHdr()
{
    LOG(INFO) << "Recover HDR Param List.";
    lc3010_->set_hdr_list(hdr_num_, camera_exposure_list_, led_current_list_, true);
...
bool Scan3D::recoverParamExposure()
{
    LOG(INFO) << "Recover Camera Exposure.";
    if (!camera_->setExposure(camera_exposure_))
    {
        LOG(ERROR) << "Recover Camera Exposure failed";
        return false;
    }
...
bool Scan3D::recoverParamGain()
{
    LOG(INFO) << "Recover Camera Gain.";
    if (!camera_->setGain(camera_gain_))
    {
        LOG(ERROR) << "Recover Camera Gain failed";
        return false;
    }
...
bool Scan3D::recoverParamLedCurrent()
{
    LOG(INFO) << "Recover Led Current.";
    if (DF_SUCCESS != lc3010_->SetLedCurrent(led_current_, led_current_, led_current_))
    {
        LOG(ERROR) << "Recover Led Current failed";
        return false;
    }
...
int Scan3D::captureFrame06Repetition_cpu(int repetition_count)
{
    ...
    if (fisher_confidence_val_ > -50)
    {
        FilterModule filter_machine_mono;
        cv::Mat maskoutput;
        filter_machine_mono.monotonicityFilter_cpu(unwrap, maskoutput, -10, fisher_confidence_val_ / 2 + 25);
    }
    ...
    int confidence = system_config_settings_machine_.Instance().firwmare_param_.confidence;
    lookup_table_machine_1.generate_pointcloud_depth_repetition(confidence,mask, unwrap, deep_map_table, xyz_mat, xL_rotate_x1, xL_rotate_y1, rectify_R11, pattern_mapping1, b);
    ...
    if (1 == system_config_settings_machine_.Instance().firwmare_param_.use_radius_filter)
    {
        ...
        LOG(INFO) << "num: " << num;
        filter_machine.RadiusOutlierRemoval_cpu(nr, nc, xyz_mat, deep_map_table, mask, 0.5, r, num);
    }
    ...
    if (1 == system_config_settings_machine_.Instance().firwmare_param_.use_depth_filter)
    {
        ...
        FilterModule filter_machine_depth;
        cv::Mat depth_map_temp(deep_map_table.rows, deep_map_table.cols, deep_map_table.type());
        cv::Mat mask_temp(deep_map_table.rows, deep_map_table.cols, CV_8UC1);
        filter_machine_depth.depth_filter_step_1(deep_map_table, depth_map_temp, mask_temp, depth_threshold / 1000.);
        filter_machine_depth.depth_filter_step_2(deep_map_table, depth_map_temp, mask_temp, depth_threshold / 1000.);
    }


    LOG(INFO) << "generate_pointcloud_base_table";
    
    deep_map_table.convertTo(deep_map_table, CV_32F, 1.0, 0.0);
    cv::Mat depth_bilateral_mat(image_height_, image_width_, CV_32F, cv::Scalar(0));
    if (1 == system_config_settings_machine_.Instance().firwmare_param_.use_bilateral_filter)
    {
        cv::bilateralFilter(deep_map_table, depth_bilateral_mat, system_config_settings_machine_.Instance().firwmare_param_.bilateral_filter_param_d, 2.0, 10.0);
        deep_map_table = depth_bilateral_mat.clone();
        LOG(INFO) << "Bilateral";
    }
    ...
    for (int r = 0; r < nr; r++)
    {
        float* ptr_w = deep_map_table.ptr<float>(r);
...
int Scan3D::captureFrame06RepetitionColor_cpu(int repetition_count) {
    if (hdr_is_shooting_.try_lock()) { lc3010_->disable_hdr_mode(); hdr_is_shooting_.unlock(); }
    LOG(INFO) << "cuda_clear_reconstruct_cache:";
    cuda_clear_reconstruct_cache();
    initCache();
    ...
    for (int r = 0; r < repetition_count; r++)
    {
        ...
        for (int g_i = 0; g_i < 16; g_i++)
        {
            ...
            if (status)
            {
                ...
                //cv::Mat img(image_height_, image_width_, CV_8UC1, img_ptr);
                if (g_i < 2 || g_i > 7) {
                    cv::GaussianBlur(channels[0], channels[0], cv::Size(5, 5), 1, 1);
                    std::cout << g_i << "gaussianblur" << std::endl;
                }
                patterns_.push_back(channels[0]);
...
//    return frame_status;
//}
int Scan3D::captureFrame06HdrColor_cpu() {
    lc3010_->enable_hdr_mode(); std::lock_guard<std::mutex> lock(hdr_is_shooting_);
    ...
    for (int hdr_i = 0; hdr_i < hdr_num_; hdr_i++)
    {
        ...
        for (int g_i = 0; g_i < 16; g_i++)
        {
            ...
            if (15 == g_i)
            {
                ...
                float b = sqrt(pow(calib_param_.translation_matrix[0], 2) + pow(calib_param_.translation_matrix[1], 2) + pow(calib_param_.translation_matrix[2], 2));
                int confidence = system_config_settings_machine_.Instance().firwmare_param_.confidence;
                lookup_table_machine_1.generate_pointcloud_depth(confidence, mask, unwrap, deep_map_table, xyz_mat, xL_rotate_x1, xL_rotate_y1, rectify_R11, pattern_mapping1, b);
    ...
    if (1 == system_config_settings_machine_.Instance().firwmare_param_.use_radius_filter)
    {
        ...
        LOG(INFO) << "radius_filter_r: " << r;
        LOG(INFO) << "num: " << num;
        filter_machine.RadiusOutlierRemoval_cpu(nr, nc, xyz_mat, depth, mask, 0.5, r, num);
    }
    ...
    if (1 == system_config_settings_machine_.Instance().firwmare_param_.use_depth_filter)
    {
        ...
        LOG(INFO) << "depth_filter_threshold: " << depth_threshold;
        FilterModule filter_machine_depth;
        cv::Mat depth_map_temp(depth.rows, depth.cols, depth.type());
        cv::Mat mask_temp(depth.rows, depth.cols, CV_8UC1);
        filter_machine_depth.depth_filter_step_1(depth, depth_map_temp, mask_temp, depth_threshold / 1000.);
        filter_machine_depth.depth_filter_step_2(depth, depth_map_temp, mask_temp, depth_threshold / 1000.);
    }


    depth.convertTo(depth, CV_32F, 1.0, 0.0);
    cv::Mat depth_bilateral_mat(image_height_, image_width_, CV_32F, cv::Scalar(0));
    if (1 == system_config_settings_machine_.Instance().firwmare_param_.use_bilateral_filter)
    {
        cv::bilateralFilter(depth, depth_bilateral_mat, system_config_settings_machine_.Instance().firwmare_param_.bilateral_filter_param_d, 2.0, 10.0);
        depth = depth_bilateral_mat.clone();
        LOG(INFO) << "Bilateral";
    }

    for (int r = 0; r < nr; r++)
    {
        float* ptr_w = depth.ptr<float>(r);
        uchar* brights = bright.ptr<uchar>(r);
        for (int c = 0; c < nc; c++)
        {
            buff_depth_[r * depth.cols + c] = static_cast<float>(ptr_w[c]);
...
int Scan3D::captureFrame06RepetitionMono12_cpu(int repetition_count)
{
    if (hdr_is_shooting_.try_lock()) { lc3010_->disable_hdr_mode(); hdr_is_shooting_.unlock(); }
    LOG(INFO) << "cuda_clear_reconstruct_cache:";
    cuda_clear_reconstruct_cache();
    ...
    for (int r = 0; r < repetition_count; r++)
    {
        ...
        for (int g_i = 0; g_i < 16; g_i++)
        {
            ...
            if (status)
            {
                ...
                cv::Mat img(image_height_, image_width_, CV_16UC1, img_ptr);
                if (g_i < 2 || g_i > 7) {
                    cv::GaussianBlur(img, img, cv::Size(5, 5), 1, 1);
                    std::cout << g_i << "gaussianblur" << std::endl;
                }
                ...
                cuda_merge_repetition_02_patterns_cpu_black(nr, nc, g_i, patterns_, merge_brightness_map, repetition_02_merge_patterns_list);
            }
            else
            {
                LOG(INFO) << "grad failed!";
    ...
    float b = sqrt(pow(calib_param_.translation_matrix[0], 2) + pow(calib_param_.translation_matrix[1], 2) + pow(calib_param_.translation_matrix[2], 2));
    int confidence = system_config_settings_machine_.Instance().firwmare_param_.confidence;
    lookup_table_machine_1.generate_pointcloud_depth_repetition(confidence*16,mask, unwrap, deep_map_table, xyz_mat, xL_rotate_x1, xL_rotate_y1, rectify_R11, pattern_mapping1, b);
    ...
    for (int r = 0; r < nr; r++)
    {
        ...
        {
            buff_depth_[r * deep_map_table.cols + c] = static_cast<float>(ptr_w[c]);
            buff_brightness_[r * merge_brightness_map.cols + c] = 0.5 + (static_cast<ushort>(bright[c]) / (repetition_count*16));
        }
    }
...
int Scan3D::captureFrame06HdrMono12_cpu()
{
    lc3010_->enable_hdr_mode(); std::lock_guard<std::mutex> lock(hdr_is_shooting_);
    ...
    for (int hdr_i = 0; hdr_i < hdr_num_; hdr_i++)
    {
        ...
        int nr = image_height_;
        int nc = image_width_;
        cv::Mat brightness_map(nr, nc, CV_8U, cv::Scalar(0));
        std::vector<cv::Mat> patterns_;
        cv::Mat threshold_map(nr, nc, CV_16U, cv::Scalar(128));
        ...
        for (int g_i = 0; g_i < 16; g_i++)
        {
            ...
            if (status)
            {
                cv::Mat img(image_height_, image_width_, CV_16UC1, img_ptr);
                if (g_i == 0) {
                    cuda_copy_minsw8_pattern_to_memory_16_cpu(nr, nc, img, brightness_map);
                }
                if (g_i < 2 || g_i > 7) {
                    cv::GaussianBlur(img, img, cv::Size(5, 5), 1, 1);
                    std::cout << g_i << "gaussianblur" << std::endl;
                ...
                cuda_handle_minsw8_16_cpu(nr, nc, g_i, patterns_, threshold_map, mask, wrap, sw_k_map, minsw_map, k2_map, unwrap);
                ...
                if (15 == g_i)
                {
                    ...
                    float b = sqrt(pow(calib_param_.translation_matrix[0], 2) + pow(calib_param_.translation_matrix[1], 2) + pow(calib_param_.translation_matrix[2], 2));
                    int confidence = system_config_settings_machine_.Instance().firwmare_param_.confidence;
                    lookup_table_machine_1.generate_pointcloud_depth_black(confidence*16,mask, unwrap, deep_map_table, xyz_mat, xL_rotate_x1, xL_rotate_y1, rectify_R11, pattern_mapping1, b);
        ...
        hdr_depth_list.push_back(deep_map_table.clone());
        hdr_brightness_list.push_back(brightness_map);
        float val = 0;
        for (int r = 0; r < nr; r++)
        {
            uchar* ptr_b = hdr_brightness_list[hdr_i].ptr<uchar>(r);
...
int Scan3D::captureFrame06Mono12_cpu()
{
    if (hdr_is_shooting_.try_lock()) { lc3010_->disable_hdr_mode(); hdr_is_shooting_.unlock(); }
    ...
    {
        float* ptr_w = deep_map_table.ptr<float>(r);
        uchar* bright = brightness_map.ptr<uchar>(r);
        for (int c = 0; c < nc; c++)
        {
            buff_depth_[r * deep_map_table.cols + c] = static_cast<float>(ptr_w[c]);
    ...
    for (int i = 0; i < 16; i++)
    {
       ...
       /* if (0 == i)
        ...
        if (15 == i)
        {
            ...
            float b = sqrt(pow(calib_param_.translation_matrix[0], 2) + pow(calib_param_.translation_matrix[1], 2) + pow(calib_param_.translation_matrix[2], 2));
            int confidence = system_config_settings_machine_.Instance().firwmare_param_.confidence;
            lookup_table_machine_1.generate_pointcloud_depth_confidence(confidence,mask, unwrap, deep_map_table, xyz_mat, xL_rotate_x1, xL_rotate_y1, rectify_R11, pattern_mapping1,b);
...
int Scan3D::captureFrame06Color_cpu()
{
    if (hdr_is_shooting_.try_lock()) { lc3010_->disable_hdr_mode(); hdr_is_shooting_.unlock(); }
    LOG(INFO) << "cuda_clear_reconstruct_cache:";
    cuda_clear_reconstruct_cache();
...
bool Scan3D::loadCalibData()
{
    ...
    {  
        ...
        // [26-03-21] 修改 "b" 为 "baseline", 提升可读性.
        LOG(INFO) << "baseline: "<< b;
